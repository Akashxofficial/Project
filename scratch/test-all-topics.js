...
Please design the MCQ, the topic summary, and the explanation strictly to explain and test the selected topics: "${topic}".
...
JSON Structure:
{
  ...
  try {
    ...
    try {
      safeJsonParse(data.text);
      console.log(`✅ Topic "${topic}" PASSED!`);
    } catch (e) {
      console.error(`❌ Topic "${topic}" FAILED!`);
    ...
    console.error(`Failed request for "${topic}":`, err.message);
  }
}
